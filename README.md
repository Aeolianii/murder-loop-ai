...
本版本重点完善“**玩家可以在合理范围内自由行动，AI 判断行动是否成立，规则系统只负责事实与因果校验**”的玩法闭环。自然语言输入和推荐行动统一进入同一条裁决链，前端会展示已确认的行动后果，而不再只显示动作名称或在开放行动上直接无响应。

自由行动链路升级为：

```txt
玩家自然语言 / 推荐行动
  -> Semantic Compiler 保留原始行为、目标和所需资源
  -> Player Specialist 根据可见事实与现实因果判断可行性
  -> 不成立：返回中文失败原因，不修改世界状态
  -> 成立：生成行动结果及必要的状态变化候选
  -> Arbiter 校验权限、证据、风险和完整因果链
  -> Reducer + Atomic Commit 写入正式状态
  -> post-commit Narrator 将已确认后果展示给玩家
```

核心变化：

- 开放行动统一使用通用 `act` 语义，由 AI 判断行为是否可行；规则层只保留手机电量、物体是否存在、场景可访问性等必要事实约束，不为每种玩家行为编写硬编码分支。
- 缺少所需物品或环境条件时，AI 会自然说明“无法完成什么、因为什么”，不会再误显示为智能叙事服务不可用，也不会凭空生成不存在的物体。
- 自然语言输入与推荐行动都会展示实际后果；检查包裹、旧书、药板、数字纸条、床底、衣柜和卫生间等目标时，会分别返回对应发现、线索或无异常结果。
- 行动叙事改为读取已确认事件：相邻动作不会错位，检查不同物品不会复用同一段结果，NPC、线索和推荐行动只能引用已经提交的事实。
- 玩家可见的行动结果、推荐按钮、失败提示和死亡结局统一使用简体中文；英文后台 operation、target 和实体 ID 不再直接暴露到界面。
- 新增通用致命行动裁决：AI 先判断资源和环境条件；条件不足时返回失败，条件成立且后果致命时生成“行动尝试 → 致命行动 → 玩家死亡 → 死亡结局”的结构化事件链。
- `self_inflicted` 死亡原因必须与玩家自身致命行动的父链一致；任何缺少可行行动、独立证据或终局父事件的死亡声明都会被规则系统拒绝。
- 死亡结果继续复用正式死亡 CG 与原子循环重置：玩家点击重来后进入下一轮，清空本轮物理状态并保留允许跨循环继承的记忆。
- 前端侧边栏新增“本回合耗时”面板，展示玩家实际等待时间、各 AI/规则/叙事环节的耗时合计和最慢环节；所有环节名称均以中文显示，并注明并行任务分别计时，因此环节合计可能大于实际等待时间。
