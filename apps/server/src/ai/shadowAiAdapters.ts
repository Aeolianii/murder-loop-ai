...
function semanticCompilerPrompt(): string {
  return [
    ...
    'Treat meaningful Chinese and English input as equally executable. Interpret English directly into canonical operations; never reject it or route it to a local fallback merely because of language.',
    ...
    'Alternative choices are not sequential actions. Treat connectors such as "or", "either...or", "或", "或者", and "还是" as alternatives in any input language. When the player offers two or more distinct state-changing alternatives without selecting one branch or giving an explicit fallback condition, return clarification_required instead of placing every alternative in orderedActions. Emit multiple orderedActions only when the player explicitly requests all actions or defines their execution order.',
    ...
    'Resolve references to physical or digital items against playerContext.availableAssets. For an existing asset used by an action, copy its exact id into inputHandleIds. Put it in communication attachmentHandleIds only when the player actually sends that asset; an instrument such as the phone used to send a message is not an attachment. Never invent an existing asset id.',
    'For an asset created earlier in the same turn, create one candidateHandles item, put its id in the producing action outputHandleIds, and reuse that exact id in every consuming inputHandleIds and communication attachmentHandleIds. A photograph produces kind="image".',
    'When a communication depends on an earlier asset-producing action and sends that result, the dependency alone is not enough: the produced handle must also be present in attachmentHandleIds.',
    ...
    'Allowed utteranceMode values: command, question, hypothetical, mixed, clarification_required, non_action. Allowed constraint type values: must_not, scope_only, conditional, sequence, visibility. Allowed ambiguity impact values: state, display, low.',
    `For utteranceMode="non_action", use this compiled brief shape: ${JSON.stringify({
      ...
      candidateHandles: [],
      ambiguities: [],
    })}. Keep the required envelope, compilerVersion, and schemaVersion fields from the normal compiled contract.`,
...
function proposalPrompt(sourceAgent: string, domain: string): string {
  ...
  return [
    ...
    'Every non-empty display fragment must cite at least one eventRefs item and at least one claimRefs item. claimRefs must include the player-visible assertion IDs from the referenced proposed events that ground the displayed text.',
    ...
    'For self-directed potentially fatal intent, decide feasibility before proposing any state transition. If a required resource or environmental condition is absent, return one blocked or failed act and must not propose death or an ending.',
    'PLAYER TERMINAL GRAPH CONTRACT: basedOnFactIds must contain only IDs from projection.factIds and must never contain canonical constraint IDs. Canonical constraint IDs belong only in evidenceRefs or invariant preconditions. Keep proposedEffects empty because the authoritative state change is represented by proposedEvents.',
    'When the intent is feasible and its direct consequence is lethal, proposedEvents must have this four-event causal shape and no substitute fields: (1) kind="action", operation="act", status="attempted", riskClass="reversible", targetIds including "player", causalParentIds=[]; (2) exactly one resolved act:completed event with kind="action", operation="act", status="completed", riskClass="irreversible", targetIds including "player", one assertion with predicate="lethality" and value="lethal" (lethality="lethal"), causalParentIds containing event 1, evidenceRefs containing event 1 plus at least one grounded projection.factIds item and invariant.death.requires_feasible_lethal_action; (3) kind="state_transition", operation="change_status", status="completed", targetIds=["player"], one assertion with predicate="status" and value="dead", causalParentIds containing event 2, evidenceRefs containing event 2 and invariant.death.requires_feasible_lethal_action; (4) kind="ending", operation="resolve_ending", status="completed", targetIds=["death"], assertions predicate="ending" value="death" and predicate="reason" value="self_inflicted" (reason="self_inflicted"), causalParentIds containing event 3, evidenceRefs containing event 3 and invariant.ending.requires_terminal_cause. Keep actorId="player" and the TurnBrief sourceActionId on all four events. Never use predicate="ending_reason". If the consequence is not lethal, do not propose death or an ending.',
...
function proposalRoleDirective(sourceAgent: string, domain: string): string {
  ...
  if (domain === 'player') {
    return 'You are the Player Specialist. You know the compiled player request, player-visible fact projection, and generic canonical constraints. Resolve every ordered player action into grounded candidate events without inventing hidden knowledge. For operation="act", decide whether the behavior is feasible: completed means it succeeds, blocked/failed means it does not. If a required resource is unavailable, preserve the request as a failed outcome instead of dropping it and do not propose a state transition or ending. For feasible self-directed fatal behavior, express the complete generic lethal-action, death, and ending causal graph required by the proposal rules. Write every player-facing event summary in Simplified Chinese and include the direct consequence; when blocked or failed, explicitly state what cannot be done and why.';
  }
  if (domain === 'killer') {
