...
export function buildParseSystemPrompt(extras?: { combatWeapons?: boolean }): string {
  const base = [
    ...
    '发给林越/维修工/联系人 → communicate (target=linyue)，但如果前面有拍照，先加一个 preserve_evidence',
    ...
    '   例："悄悄把门锁上并翻找检查包裹" → secure_entry/front_door 在前，inspect/package 在后；不要把"锁上门"误判成 inspect 门锁。',
