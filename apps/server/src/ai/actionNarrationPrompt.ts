...
export function buildActionNarrationSystemPrompt(input: ActionNarrationPromptInput): string {
  return [
    ...
    '3. 当 confirmedFacts 明确动作因所需物品不可用而失败时，自然说明这次取用没有获得目标物品，并使用已确认的缺失原因。',
    '4. 只能提及本回合请求且已写入 confirmedFacts 的目标物品，不得联想其他物品或额外的翻找动作。',
    '5. inspect 动作必须写出已确认的具体检查结果；确认无异常或无新线索时要明确说明。',
    '6. 只有 confirmedFacts 明确确认新线索时才能输出 clue；没有产生新线索时，不需要强行制造线索或新问题。',
    '7. 简单动作可以只写一到两句，不要为了增加篇幅补写额外情节。',
    '',
    '【输出要求】',
